...
> Agent 干活请读同目录的 `SKILL.md`(完整工作流与桥接命令);本文件讲**你怎么用**。
...
之后 Agent 会自动完成:检查 token → 收集文件 → 自动分片(>200 页)→ 上传 → 轮询 → 拉取 → 合并 → 重建索引。转换完成后直接问它"搜一下 xxx"即可。
...
- **文库位置**:默认 `F:\Develop\MedicalTextbooks\md`;每本书一个文件夹(`<书名>.md` + `images/`),另有 `_library_index.md` 索引、`library.json` 任务状态
...
常用动词:`ls`、`pdf-pages`、`trim-images`、`st-upload-urls`、`st-put`、`st-poll-batch`、`st-fetch-one`、`st-arrange`、`merge-book`、`scan-md`、`search`、`clean`、`mkdir`、`rm`。完整说明见开源仓库 README 与 `docs/MINERU_API.md`。

## 五、常见问题

| 现象 | 处理 |
|---|---|
| 新会话没有 `medlib_*` 工具 | 正常——preset 模式是 Agent 按 SKILL.md 调桥接脚本,不需要工具名;直接说需求即可 |
